...
CREATE PROCEDURE [mtd].[master_gen_sp_drop_rdv] (@execute_sql INT = 0)
AS
BEGIN
	...
	SELECT 
		replace(g.core, '<table_name>', h.table_name) as command
	FROM [mtd].[master_generator] g
	CROSS JOIN adf.meta_hub_mapping h
	WHERE generator_type = 'sp_drop_hub_table'
	  AND h.active_ind = 1 
	GROUP BY g.core, h.table_name, h.schema_name 
	UNION ALL
	SELECT 
		replace(g.core, '<table_name>', h.table_name) 	
	FROM [mtd].[master_generator] g
	CROSS JOIN adf.meta_hsat_mapping h
	WHERE generator_type = 'sp_drop_hsat_table'
	  AND h.active_ind = 1 
	GROUP BY g.core, h.table_name, h.hub_table_name, h.schema_name 
	UNION ALL
	SELECT 
		replace(g.core, '<table_name>', h.table_name) 	
	FROM [mtd].[master_generator] g
...
INSERT INTO @sql_table
	...
	from [mtd].[master_generator] g
	cross join adf.meta_hub_mapping h
	where g.generator_type = 'sp_drop_hub_table'
	  and h.active_ind = 1 
UNION ALL
	SELECT -- hsat
		replace(
	...
	from [mtd].[master_generator] g
	cross join colist h
	where g.generator_type = 'sp_drop_hsat_table'
UNION ALL
	SELECT 
		replace(g.core, '<table_name>', h.table_name) 	
	FROM [mtd].[master_generator] g
